...
import { message, Button, Input, List, Card, Tag, Spin, Row, Col, Typography, Divider, Form, Alert, Space } from 'antd';
import { PlayCircleOutlined, PauseCircleOutlined, SendOutlined, SaveOutlined } from '@ant-design/icons';
...
const EditorPage: React.FC = () => {
    ...
    const handleWrite = async () => {
        ...
        try {
            await api.post(`/novels/${novelId}/write`, { prompt: currentPrompt });
            fetchNovelAndHistory(); // Refetch everything to get the latest state
        } catch (err) {
    ...
    if (chapterId) {
        return (
            <Card>
                <Title level={2}>Edit Chapter</Title>
    ...
    const handleStart = async () => {
        try {
            ...
            setError('Failed to start the writing process.');
        }
    };
    ...
    const handlePause = async () => {
        try {
            ...
            setError('Failed to pause the writing process.');
        }
    };
        ...
        <Row gutter={24}>
            <Col span={16}>
                <Card>
                    ...
                    <Paragraph><strong>状态:</strong> <Tag color={novel?.status === 'Writing' ? 'green' : 'volcano'}>{novel?.status}</Tag></Paragraph>

                    <Space style={{ marginBottom: '24px' }}>
                        <Button type="primary" icon={<PlayCircleOutlined />} onClick={handleStart} disabled={novel?.status === 'Writing'}>开始自动写作</Button>
                        <Button icon={<PauseCircleOutlined />} onClick={handlePause} disabled={novel?.status !== 'Writing'}>暂停自动写作</Button>
                    </Space>
                </Card>
                        ...
                        <List
                            ...
                            renderItem={item => (
                                <List.Item key={item.id}>
                                    <HistoryItemView
                                        item={{ ...item, agentName: item.agent?.name ?? '用户' }}
                                        novelId={novelId}
